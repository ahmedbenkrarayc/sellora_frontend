<template>
  <div class="flex-1 overflow-y-auto p-4 md:p-8 pb-16">
    ...
    <h2 class="text-[13px] md:text-[14px] text-[#636363]">Update your products effortlessly and stay ahead!</h2>
    ...
    <section class="w-full rounded-lg p-3 md:p-4 mt-4 md:mt-6">
      <div v-if="productStore.loading" class="flex justify-center items-center py-12">
        <div class="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#493D9E]"></div>
      </div>
      
      <div v-else-if="productStore.error" class="bg-white px-4 py-6 sm:px-8 md:px-12 lg:px-16 lg:py-12 rounded-lg">
        <div class="text-red-500 text-center">
          <p>{{ productStore.error }}</p>
          <button 
            @click="goBack" 
            class="mt-4 px-6 py-2 bg-[#493D9E] hover:bg-[#42369e] text-white rounded-md text-[14px] font-medium"
          >
            Go Back
          </button>
        </div>
      ...
      <div v-else class="bg-white px-4 py-6 sm:px-8 md:px-12 lg:px-16 lg:py-12 rounded-lg">
          <h3 class="font-medium text-[19px] text-[#222222] mb-12">Update product</h3>
          ...
          <div class="flex justify-between mt-6">
            <button 
              @click="goBack" 
              class="px-6 py-2 bg-gray-300 hover:bg-gray-400 text-gray-800 rounded-md text-[14px] font-medium"
            >
              Cancel
            </button>
            ...
            <button 
              ...
              :class="[
                'px-6 py-2 w-fit text-white rounded-md text-[14px] font-medium',
      ...
      <div class="p-4">
        <div v-if="selectedVariantImages.length > 0" class="mb-6">
          ...
          <div class="flex flex-wrap gap-4">
            ...
            >
              <img :src="getImageUrl(image)" class="w-full h-full object-cover rounded" :alt="`Variant image ${index + 1}`" />
              <div 
                v-if="index === 0" 
...
async function loadProductData() {
  try {
    ...
    if (product) {
      ...
      sizes.value = product.variants
        .map(v => v.size.name)
        .filter((v, i, a) => a.indexOf(v) === i)
      if (sizes.value.length === 0) sizes.value = ['']
      
      colors.value = product.variants
        .map(v => v.color.name)
        .filter((v, i, a) => a.indexOf(v) === i)
      if (colors.value.length === 0) colors.value = ['']
      
      additionalDetails.value = product.product_details.map(d => ({
        key: d.key,
        value: d.value
      }))
      if (additionalDetails.value.length === 0) {
      ...
      productVariants.value = product.variants.map(v => ({
        ...
        price: v.price,
        stock: v.stock,
        availability: v.available.toString(),
        images: v.images.map(img => ({
          id: img.id,
          path: img.path,
          url: `${imagesUrl}${img.path}`
        }))
      }))
    }
  } catch (err) {
...
function getImageUrl(image) {
  ...
  if (image.path) return `${imagesUrl}${image.path.split('/').pop()}`
  return ''
}
...
function generateVariants() {
  ...
  const existingVariants = [...productVariants.value]
  const newVariants = []
...
async function updateProduct() {
  ...
  const variantsForBackend = productVariants.value.map(variant => {
    return {
      ...variant,
      images: variant.images.map(img => img.file || img)
    }
  })
  ...
  try {
    await productStore.updateFullProduct(productId.value, payload)
    toast.value.add({ 
      severity: 'success', 
    ...
    if (err.response && err.response.status === 422) {
      ...
      toast.value.add({ 
        severity: 'error', 
        summary: 'Error', 
        detail: productStore.error || 'An error occurred while updating the product', 
        life: 5000 
      })
    }
  } finally {
